# cmux-linux — Crash Course / Refresher
...
Runs *after* render. The **dependency array** controls when. The **return function**
...
function TerminalPane({ paneId }) {
  ...
  useEffect(() => {
    ...
    return () => { off(); term.dispose(); }; // cleanup
  }, [paneId]);
...
server.listen("/tmp/cmux-linux.sock");
```
A client (your `cmux` CLI) just connects to the same path and writes JSON.

### `child_process` — running programs
node-pty handles the *terminals*, but for one-off commands (e.g. `git branch`) you
...
```jsonc
{
  ...
  "dependencies": {     // shipped at runtime (xterm, node-pty)
    "node-pty": "^1.x", "@xterm/xterm": "^5.x"
  },
  "devDependencies": {  // build-time only (electron, vite, typescript)
    "electron": "^3x", "electron-vite": "^2.x", "typescript": "^5.x"
  }
}
```
...
`electron-vite` just runs Vite for the renderer *and* bundles the main/preload too.
...
### The app shell
```css
.app   { display: flex; height: 100vh; }      /* sidebar | main, full height */
.sidebar { width: 260px; flex: 0 0 260px;     /* fixed width, don't grow/shrink */
           overflow-y: auto; }
.main  { flex: 1 1 auto; min-width: 0;        /* takes the rest; min-width:0 lets it shrink */
...
```css
.panes      { display: flex; flex: 1; }            /* horizontal split */
.panes.vertical { flex-direction: column; }        /* vertical split */
.pane       { flex: 1 1 0; min-width: 0; min-height: 0; } /* equal panes */
```
...
.row.active { background: #1f6feb; border-radius: 6px; } /* blue highlight */
.row .name   { font-weight: 600; }
.row .status { font-size: 12px; opacity: .6; }          /* dim subtitle */
```
