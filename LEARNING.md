# cmux-linux — Learning Plan (Electron, from scratch)
...
*Needed from M0 onward. The template is TS; you mostly just add type annotations.*
...
- 📚 *TypeScript Handbook → "The Basics" + "Everyday Types"; the React+TS cheatsheet*
...
*This is THE foundational concept. Needed for M0–M1. Most important block.*

- [ ] **Main process vs Renderer process** — what runs where, and why they're separate
      - Main = Node, full OS access ("the backend")
      - Renderer = Chromium tab running your React app ("the frontend"), sandboxed
...
- [ ] Mental model lock-in: *main = Express backend, renderer = React frontend*
- 📚 *Electron docs → "Process Model" + "Tutorial" parts 1–2*
- 🛠 **Exercise:** open a window that loads a static React page. (This basically = M0.)

---

## Block 2 — IPC: how the two processes talk ⏱ ~3–4 hrs
*The single most important Electron skill for this app. Needed for M1.*
...
- 📚 *Electron docs → "Inter-Process Communication" (read the whole page)*
...
*The safe way to expose main-process functions to React. Needed for M1.*
...
- 📚 *Electron docs → "Context Isolation" + the preload section of the IPC page*
- 🛠 **Exercise:** expose `window.api.getUsername()` from preload; call it in React.

---

## Block 4 — node-pty (real shells) ⏱ ~2–3 hrs
*The heart of a terminal app. Needed for M1.*
...
- 📚 *node-pty README (it's short); skim its examples folder*
- 🛠 **Exercise (in main only):** spawn bash, write `ls\n`, log the output.

---

## Block 5 — xterm.js (the terminal UI) ⏱ ~3–4 hrs
*Renders the terminal in the browser. Needed for M1, deepened in M2/M6.*
...
- 📚 *xterm.js docs/guides + the "getting started" example*
...
*Project scaffolding + dev experience. Needed at M0.*
...
- 📚 *Electron docs → "Security" checklist*
...
Node, and CSS work you already know — you learn the rest *as you hit it*.
...
5. Keep the mental model: *main = backend, renderer = frontend, IPC = the API.*
