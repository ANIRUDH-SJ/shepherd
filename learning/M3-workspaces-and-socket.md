...
> sidebar from a terminal. This is the milestone that makes it *feel* like cmux.
...
```
 ...
 ───────────────────           ─────────────                   ────────
 cmux set-status "building"  →  socket server  →  IPC        →  appReducer
   (bin/cmux, over the           (net, JSON)     socket:command   setStatus
...
│   ├── pty.ts               ~ inject CMUX_* env into each pane
...
bin/cmux                      ★ the CLI client agents call
```
...
### `main/socket.ts`
A `net` server on a **unix socket** (`/tmp/cmux-linux.sock`, override
`CMUX_SOCKET_PATH`) speaking **newline-terminated JSON** `{id, method, params}`:
...
- **The mirror pattern:** the workspace list lives in the *renderer*, but the socket
  server (in *main*) needs it to resolve `--workspace <id|name>` and to default to
...
Every pane's shell gets `CMUX_WORKSPACE_ID`, `CMUX_SURFACE_ID`, `CMUX_SOCKET_PATH`,
and `<cwd>/bin` on `PATH`. So when you run `cmux set-status …` **inside** a pane, it
...
the reply. It defaults `--workspace` to `$CMUX_WORKSPACE_ID`. This is what an agent
...
- **A CLI/protocol test** — ran `bin/cmux` against a mock socket server and asserted
  it sends the right JSON (status text + `CMUX_WORKSPACE_ID`), round-trips `ping`, and
  prints `list-workspaces`. (Caught a fun self-deadlock in the *test* first: a
...
- **`PATH` injection assumes dev** (`<cwd>/bin`); packaging wires it properly in M6.
  Fallback: run `node bin/cmux …` with `CMUX_SOCKET_PATH` set.
...
- **Drive the sidebar from a terminal** — in any pane, run:
  ```bash
  cmux set-status "building the app…"     # → that workspace's subtitle updates
  cmux notify --title Deploy --body "done"  # → the workspace flashes + a desktop toast
  cmux list-workspaces
  ```
  (If `cmux` isn't found on `PATH`, use `node bin/cmux …` — `CMUX_SOCKET_PATH` is
...
3. The socket server is in *main*, but workspaces live in the *renderer*. How does
   the server resolve `--workspace` — and why the "mirror"?
4. What does env injection put into each pane, and why does `cmux set-status` work
...
We wire **real agent hooks** (Claude Code → `cmux notify`), add **OSC 9/99/777**
auto-detection so *any* program can raise the sidebar, and add **session restore**
