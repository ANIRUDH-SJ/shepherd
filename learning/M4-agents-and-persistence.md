...
> **The goal of M4:** make the app work with _real agents_ and _survive a restart_.
> Three features: (1) **OSC auto-notifications** — any program's escape codes flash
> the sidebar; (2) **agent hooks** — `shepherd hooks setup` wires Claude Code's
...
By M3 the sidebar could flash via the socket (`shepherd notify`). M4 adds a **second,
...
```
 ...
 (b) Claude Code fires its Notification hook          ─┤    → sidebar flash + toast
     → runs `shepherd notify` (installed by `hooks setup`)─┘
```

`(a)` needs nothing; `(b)` is one `shepherd hooks setup`. Both reuse the M3 `notify`
...
`SHEPHERD_WORKSPACE_ID`) using the same `notify` path, plus a desktop toast.

> **🔧 In Shepherd:** try it — `printf '\033]9;hello\007'` in a pane flashes that
> workspace. That's a program telling the terminal "notify me" with no Shepherd CLI.

---

## 3. Agent hooks (PR #3)

### `bin/shepherd hooks setup`
    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "shepherd notify --title Claude --body \"needs your attention\""
          }
        ]
      }
    ]
...
- **Fail _closed_ on a bad snapshot.** The first cut only checked the root's
...
- **Persist only the layout, gated on change.** The save fired on _every_ state
...
- **Hooks:** run `shepherd hooks setup`, then run Claude Code in a pane — it flashes on
