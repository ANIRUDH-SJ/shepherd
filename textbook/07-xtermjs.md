...
> what a terminal _shows_; Chapter 6 (`06-node-pty.md`) for the real shell whose
...
> **🔧 In Shepherd:** xterm.js is the "GPU terminal" box in the Chapter 1
...
the terminal must _be_ the shell."
...
you paint and reports whatever you type — and _something else_ has to be the
brain that connects the two. In Shepherd, that brain is the round-trip through
...
> Terminal wired to _nothing_ looks broken in a very specific way: you type and
...
call this. (There's a subtle but important rule about _when_ the container must
...
anything on screen. In Shepherd, essentially every `write` call's argument comes
...
shell's _output_; xterm's `onData` is the user's _input_. (Keep them straight: in
...
> **🔧 In Shepherd:** map these four onto the loops from Chapter 1.
> `term.write(data)` is the _renderer_ end of **Loop B** (paint the shell's
> output). `term.onData(data)` is the _renderer_ start of **Loop A** (the user
...
after mounting and again on every layout change. This is _the_ addon that connects
...
> **⚠️ Gotcha — WebGL context loss.** A browser can _revoke_ a WebGL context at
...
- **The 16 ANSI colors** (`black`…`white` and their 8 `bright*` variants) are the
  palette a _program's_ output draws from. When `ls` prints a directory in blue or
...
> **🔧 In Shepherd:** the terminal theme is one half of our dark look; the
...
> **⚠️ Gotcha:** update the theme by assigning a _new_ object to
...
its own render loop. React's job is _not_ to render the terminal's contents —
...
here's the pattern and the _why_.
...
- **Creation happens in `useEffect`, not during render.** `term.open()` needs a
  ...
  runs _after_ the DOM is painted, so `boxRef.current` is a real element by then.
...
_pixels_) and the main process (which owns the _shell_). We saw the main-process
...
> **🔧 In Shepherd:** because our panes tile and drag (`10-tiling-and-layout.md`),
...
function BadTerminal({ ptyId }) {
  ...
  useEffect(
    () =>
      window.api.onPtyData(
        ptyId,
        (chunk) => setOutput((prev) => prev + chunk) // state grows on every chunk
      ),
    [ptyId]
  )
...
and never re-renders.** Output bytes go _straight_ into `term.write()`, bypassing
...
> state or props.** React owns _whether a pane exists and where it sits in the
> layout_; xterm.js owns _what's inside it_. The only React state near a terminal
...
`ptyProcess.write('l')` in main, Chapter 6). You do _not_ see the `l` yet — it's
...
over IPC, our `window.api.onPtyData` handler runs `term.write('l')`, and _now_ the
...
> **🔧 In Shepherd:** `window.api.onPtyData`, `sendInput`, `resizePty`, and
...
> _how many_ `TerminalPane`s exist and _where_ they sit. Each one owns exactly one
...
1. In one sentence each: what are the _only two_ things xterm.js does, and which
...
5. Name the two resources a WebGL-backed Terminal leaks if you forget
   `term.dispose()`, and why the second one eventually breaks _new_ terminals.
...
(`term.onData`), and does _nothing else_: it has no shell, runs no commands, and
...
never through React state, because React owns _whether a pane exists_ while
xterm.js owns _what's inside it_. Wired to node-pty over IPC, `TerminalPane` is the
