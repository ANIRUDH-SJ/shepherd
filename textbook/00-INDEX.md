...
Welcome. This folder is a **from-scratch textbook** that explains *everything*
...
concept. If you read it in order, you will understand not just *what* the code
does but *why* every piece exists.
...
> the hood. Every abstract concept is tied back to *our actual app*.
...
3. **Read it beside the build docs.** This book explains *concepts*; `ROADMAP.md`
   is the *build order* and `FEATURES.md` is *what we're building*. They cross-link.
...
- `17-how-it-all-connects.md` — the capstone: we trace a single keystroke *and* an
...
**What we're building:** *cmux-linux* — a Linux desktop app that recreates
...
milestone, we'll deepen the relevant chapter with real snippets from *our* code,
